...
- **Lemma 3.6 ([T26]).** (i) For `F ∈ 𝓧`, the map `τ ↦ β_d(v_τ)F|_{I_+}` is continuous on `𝕊_{iπ}`
  ...
  which is continuous and holomorphic on the same domain. Definition 3.1's continuity/holomorphy
  clause (Φ1) is stated for exactly this scalarised family, not for the `D`-valued map of clause
  (ii), so no `D`-valued formalisation is needed for that requirement; (Φ1)'s boundary identity
  and (Φ2) are established separately.
