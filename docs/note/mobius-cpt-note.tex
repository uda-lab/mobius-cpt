...
すなわち $P(I_\pm)\Omega$ の稠密性（\cite{CRTT25} 付録 A の系 A.3 (i)）も形式化した。
定理 3.10 の (i)--(iii) 自身はこれを用いないが、\cite{T26} が定理 3.10 の前文で
$\Vt_{\pm i\pi}$ を「稠密に定義された」作用素と呼ぶとき、その語を支えているのが
この稠密性だからである。次のものは扱わない。Haag--Kastler の意味での共形ネットとその頂点作用素代数との
対応（\cite{CKLW18}）、
...
作用素の順序は逆転している。(i)--(iii) はいずれも稠密性を主張していない。「稠密に定義された」
という語は \cite{T26} の定理 3.10 の前文に現れるが、それは $P(I_\pm)\Omega$ が $\Dom$ の中で
$\Fields$-強位相について稠密であるという別の事実、すなわち Reeh--Schlieder 性
（\cite{CRTT25} 付録 A の系 A.3 (i)）を指すものであり、(i)--(iii) のどれもそれに依存しない。
この稠密性は $I_\pm$ について独立に成り立つ定理であり、それも検証されている
（第 \ref{sec:status} 節）。両者を合わせると、$\Vt_{\pm i\pi}$ が実際に稠密に定義された
作用素であることが従う。
...
\cite{CRTT25} 補題 2.10 (i) & $t \mapsto V_t\Phi$ の $\Fields$-強連続性 \\
