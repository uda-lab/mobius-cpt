...
$e^{N\abs{x}}$ で重みづけした $C^N(\mathbb{R})$ の半ノルムと\textbf{同値}である。
一致ではない。$1$ 階でも重みはちょうど $\cosh x$ であり、この作用素を繰り返せば
低階の導関数の項も出る。証明が使うのは、両者が互いに定数倍で押さえ合うことである。そして最も重要な点として、
\textbf{端点 $\pm 1$ における全階平坦性は $\operatorname{Re} x \to \pm\infty$ における
超指数減衰にほかならない}。
