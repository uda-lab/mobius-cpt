...
述べ、主定理を正確に述べたうえで、証明の依存関係を図示する。第 4 節は $\Xcl$ から作られるベクトルの上での
...
以下で本質的に用いられる特徴が二つある。第一に、和が $j = 0$ から始まるので
...
固有値とみる立場で、$L_0$ のスペクトルの正値性を内積を使わずに述べたものである。
...
部分空間として挙げるのは $P_\Xcl(I_+)\Omega$、すなわち $\Xcl$ から作られるベクトルの
全体であり、その稠密性は Reeh--Schlieder 性（$P(I_\pm)\Omega$ が $\Dom$ の中で
...
試験関数だけで作られるベクトル $\varphi_1(f_1)\cdots\varphi_k(f_k)\Omega$ であり、その全体は
第 \ref{sec:core} 節で $P_\Xcl(I_+)\Omega$ と書かれる。この段階では
...
\node[box, below=7mm of l36] (l37) {補題 3.7\\ $\Xcl$ から作られる\\ ベクトル上の PCT 公式};
...
\section{\texorpdfstring{$\Xcl$}{X} から作られるベクトルの上での PCT 公式}
\label{sec:core}
...
みなしたときの全階の実偏導関数が $\pm 1$ で消えるという読み方である。$\Oex$ 上 $C^\infty$ 級で
...
第二に、二つの積の差を評価するときの展開の取り方が効く。
$\varphi_1(f_1)\cdots\varphi_k(f_k)\Omega - \varphi_1(g_1)\cdots\varphi_k(g_k)\Omega$ を、
先頭から $j-1$ 個を $g$、$j$ 番目を $f_j - g_j$、残りを $f$ とした $k$ 個の項の和に分ける。
...
$\beta_d(r_\pi)$ と反転の可換性から、もとの列を逆順にし試験関数を引き戻した符号つきの積に
正確に着地する。
