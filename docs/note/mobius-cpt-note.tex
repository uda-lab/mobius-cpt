...
defined on every product of fields smeared inside the upper semicircle, and there it reverses
the order of the product, reflects each test function through $z \mapsto z^{-1}$, and multiplies the
...
検証の意味を述べ、第 8 節で限界と未着手の項目を挙げる。定義・補題・定理の番号は
\cite{T26} のものに合わせてあり、本稿独自の項目には英字を付す。番号は原論文のものだから
連続せず、また原論文の項目のうち本稿が番号で引用するだけで改めて述べないものもある。
...
閉帯上で一様な評価が必要であり、それが補題 3.8 と補題 3.9 である。
...
\draw[flow] (ax.east) to[out=0,in=0] (l38.east);
\draw[flow] (l38) -- (l39);
\draw[flow] (l39) -- (thm);
...
直接得られる。
補題 3.9 は補題 3.8 と、補題 3.7 の両方の主張（下側境界に (i)、上側境界に (ii)）を
使う。補題 3.4 は、補題 3.7 が扱う試験関数の族が稠密であることを保証し、
...
\cite{T26} 補題 3.6 (i) & $\tau \mapsto \beta_d(v_\tau)F|_{I_+}$ の $\Strip_{i\pi}$ 上の連続性と \\
 & 内部での正則性 \\
(補題 3.6 (ii) の & 各 $G_\lambda$、すなわち \eqref{eq:Glambda} の $\Strip_{i\pi}$ 上の \\
\ スカラー化した帰結) & 連続性と内部での正則性 \\
