...
これとは別に、開発には定理 3.10 に至る主張を原論文の語彙のまま書き並べた一覧があり、
そこには公理系を満たす理論そのものが、まだ与えられていない対象として一つ置かれている。
置かれているのは「その対象が与えられていない」ことを型の上で表明する項であって、対象
...
論理的基礎をなす。本開発はこれ以外の公理を導入していない。導入すれば、この検査が
その時点で失敗する。したがってこの検査が排除しているのは、本開発に固有の公理の追加や
それによる検証の迂回であって、上の三つを含む基礎そのものではない。
