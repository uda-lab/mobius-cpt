...
% If no route is available in full the build stops and names all three.  See README.md.
%
% Two properties are wanted at once.  (a) A route is taken only when EVERY face it needs is
% present, so that a half-installed route -- an interrupted download into \jafontdir, a
% partial manual font installation -- falls through to the next route instead of being
% selected and then failing on the missing face.  Hence one probe per face, any failing
% probe dropping the whole route -- the document sets Japanese in bold as well as in the
% regular weight, so a route offering only the regular faces is incomplete.  (b) A later
% route is probed only if the earlier ones did
% not succeed, so that the reference build touches no font outside \jafontdir; probing by
% family name reaches into system font directories, which makes the build environment
% dependent and is reported as such.  Hence the probes are guarded, not run up front.
\newif\ifjafontA \newif\ifjafontB \newif\ifjafontC \newif\ifjafontset
\jafontsetfalse
...
\ifjafontset\else
  ...
  \IfFontExistsTF{Noto Serif CJK JP Bold}{}{\jafontCfalse}
  \IfFontExistsTF{Noto Sans CJK JP}{}{\jafontCfalse}
  \IfFontExistsTF{Noto Sans CJK JP Bold}{}{\jafontCfalse}
  \ifjafontC
    ...
    \setCJKmonofont[BoldFont={Noto Sans CJK JP Bold}]{Noto Sans CJK JP}
    \jafontsettrue
  \fi
\fi
...
\ifjafontset\else
  \PackageError{mobius-cpt-note}{No complete Japanese font set available}{%
    ...
    Regular and Bold, installed system-wide and visible to fontconfig.}
\fi
...
\author{Tomoki Uda\thanks{Faculty of Science and Technology, Nanzan University.
  ORCID: \href{https://orcid.org/0000-0002-6073-1572}{0000-0002-6073-1572}.}}
\date{\notedate}
...
開発リポジトリそのものへのアクセスは前提としない。もっとも、同リポジトリは
\url{https://github.com/uda-lab/mobius-cpt} で公開されている。上のコミット識別子は
版を一意に指定するから、そのソースと以下の記述との対応は一意に定まる。
...
検査は失敗する。公理の宣言そのものを禁じる別の検査もある。この検査には、所有者が
明示的に承認した場合の例外が用意されているが、固定した版でその例外は一つも使われて
いない。したがってこの二つが排除しているのは、
...
\section*{謝辞}
元良直輝氏には、本研究の出発点となった数学ノートをご提供いただいたことに感謝する。
