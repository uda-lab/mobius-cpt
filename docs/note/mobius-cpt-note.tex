...
\medskip
\begin{center}
\textit{\small 研究ノート（暫定版）。査読を経たものではない。}
\end{center}
...
$0 \le \operatorname{Im}\tau \le \pi$ なる帯状領域上で定義され、上半円 $I_+$ に台をもつ
...
$z \mapsto z^{-1}$ で引き戻されている。この二つの反転と符号 $(-1)^d$ が、unitary な場合の
PCT 作用素の作用に対応する。
...
複素数値可微分関数の空間を表す。$z = e^{i\theta}$ による角変数 $\theta$ に関する導関数を
用いて
\[
...
\]
は $\pm 1$ を固定し、$-1$ から $1$ へ向かう流れを与える。$\Mob$ の複素化 Lie 環は
...
$\lambda(\Phi) \ne 0$ なる $\lambda \in \Dstar$ が存在することをいう。$\Dom$ 上の
...
これは $f$ が反対側の\emph{開}半円 $I_\mp$ 上で恒等的に $0$ になることを意味する。
...
その理由を述べる。$C_0^\infty(I_\pm)$ で、$I_\pm$ 上可微分で端点 $\pm 1$ において
\emph{すべての階の導関数が $0$ に収束する}関数の全体を表す。零拡張により
...
を、虚部が $0$ と $\operatorname{Im}\tau$ の間にある点のなす閉じた帯状領域とする。
...
原論文の記法に従う。別名として $\Dom_\tau := \Dom(\Vt_\tau)$ も用いる。
\end{remark}

この定義が意味をもつためには二つの一意性が必要である。$\Psi$ の一意性は正則性から従う。
...
\label{thm:main}
$(\Dom, \Fields, U, \Omega)$ を M\"obius 共変 Wightman 共形場理論とする。
\begin{enumerate}
...
\label{def:Xcl}
$\Xcl$ を、$\Oex$ 上可微分でその内部において正則、$F(\infty) = 0$、かつすべての $i \ge 0$ に
...
\begin{equation}
  ...
  \qquad (z \in I_+)
\end{equation}
を複素パラメータ $\tau \in \Strip_{i\pi}$ に対する\emph{定義}として採用できる
...
\end{equation}
を与える。反転が二つの半円を交換するから、右辺に現れるのは $F$ の\emph{下}半円への
制限である。
...
（$N = 0$ のとき定数はちょうど $e^{\abs{t}}$ である）。二つを合わせるには、
...
閉じた帯上での連続性はコンパクト近傍上の一様収束から従う。境界の同定は二つある。
...
符号・順序反転公式と補題 \ref{lem:38} を反転した場の列に適用して、極限が
...
自明でないのは、$\mathbb{R}$ が $\Strip_\tau$ の\emph{内点でない}という点である。一致の定理は
...
鏡像原理を使わずに済ませることができる。二つの族の差 $g$ は $\mathbb{R}$ 上\emph{恒等的に $0$}
...
形式化においては、参照した mathlib に鏡像原理が存在しなかったため、この経路が
選ばれた。Cauchy--Goursat と Morera と一致の定理はいずれも利用可能であった。

\subsection{共形次元 $0$ における可除特異点}
\label{sec:d0}
...
極は可除である。$F$ は $\infty$ で可微分で $F(\infty) = 0$ だから $w \mapsto w F(w)$ は
...
閉じた上半円周上で $\Strip_{i\pi}$ のパラメータに対して $P \ne 0$ であり、
消えうるのは $Q$ のみで、その消滅がちょうど極 $v_{-\tau}\cdot z = \infty$ に対応する。
...
この座標には二つの利点がある。第一に、\cite{T26} の「ブースト流に沿った Gauss 核との
畳み込み」が、実直線上の通常の Gauss 畳み込みそのものになる。第二に、
$\partial_\theta = -\cosh(x)\, \partial_x$ だから、$C^N(\Circ)$ の半ノルムは
$e^{N\abs{x}}$ で重みづけした $C^N(\mathbb{R})$ の半ノルムと一致する。そして最も重要な点として、
\textbf{端点 $\pm 1$ における全階平坦性は $\mp\infty$ における超指数減衰にほかならない}。
補題 \ref{lem:34} の近似列が端点を越えて可微分であることは、この言い換えのもとで
証明できる主張になる。
...
正則性は、平坦性から可微分性への延長の議論が消費するものだから、この選択は
本質的である。

構成そのものは次のようになる。$f$ を上半円に台をもつ元とし、$a$ を $zf$ のこの座標での
像とする（平坦性の言い換えにより $a$ は急減衰する）。$H_s = g_s * a$ をその Gauss 平滑化と
すると、Gauss 核が整関数だから $H_s$ は整関数であり、近似列を
...
外で $1$、$\varepsilon/2$ 近傍の内で $0$ となる可微分な切断関数 $\chi_\varepsilon \in C^\infty(\Circ)$ を
...
主張するのではなく、各 $\lambda$ について数列の収束を示してから正則性で戻る。
...
そこで初めて効く。$F(\infty) = 0$ という定義 \ref{def:Xcl} の条件は、この零モードの
消滅のためにある。
...
あり、ベクトル値積分を要求するから採れない。分離正則性と同時連続性から対角正則性を
導く Osgood の定理を使う道は、$k$ 変数の反復 Cauchy 公式を要する。

同じ理由から、分離連続性と同時連続性の区別にも注意が要る。$C^\infty(\Circ)$ が
Fr\'echet 空間であることから両者は多重線形写像について一致する（barrelledness による）が、
この同値は一般の局所凸空間では成り立たない。両立性の定義を分離連続性で述べるか
同時連続性で述べるかは、したがって選択の問題ではなく、Fr\'echet 性に依存した定理である。

\subsection{ブーストの $C^N$ 増大度}
\label{sec:growth}
...
\le e^{\pi^2} e^{-(\operatorname{Re}\tau)^2}$ だから、$\mathfrak{h}$ が帯の上で
（$F_j, G_j$ に依存してよい定数で）有界でありさえすれば、$e^{-\tau^2}\mathfrak{h}(\tau)$ は
$\operatorname{Re}\tau \to \pm\infty$ で $0$ に収束する。したがって水平帯に対する
Phragm\'en--Lindel\"of の定理が適用でき、最大値は境界で達成される。結論の
$e^{(\operatorname{Re}\tau)^2}$ という因子は、この重みを外した残りである。

必要な入力は三つある。閉帯上の連続性と内部での正則性、帯の上での有界性、そして
二つの境界線上での評価である。第一のものは補題 \ref{lem:37} (i) と補題 \ref{lem:36} から、
...
場の列を反転し試験関数を $z^{-1}$ で引き戻した積に書き換えたうえで、反転した列に
補題 \ref{lem:38} を適用して得られる。このとき現れる積と最大値の量が、もとの列に
対する量と\emph{等しい}ことを確かめる必要がある。単に押さえられるだけでは、
両境界の評価を統一するときに定数が制御できない。等号は \eqref{eq:cnorm-inv} と
反転の線形性、および積と最大値が列の並べ替えで不変であることから従う。

\subsection{符号 $(-1)^k$ の出所}
\label{sec:sign}
...
パラメータを反転する。すなわち $r_\pi v_t r_\pi^{-1} = v_{-t}$ が符号の不定性なく
成り立つ。この行列としての等式から、$U(r_\pi)(V_t \Phi) = V_{-t}(U(r_\pi)\Phi)$ が従う。
また $r_\pi$ による試験関数の引き戻しは、すべての共形次元 $d$ に対して
$\beta_d(r_\pi)$ に一致する（回転の共形因子が $1$ だからである）。この引き戻しは
...
$\beta_d(r_\pi)$ と反転の可換性から、もとの列に対する反転・順序反転・符号つきの積に
正確に着地する。

「一般性を失うことなく」が正当であることは、この一連の対応が実際に構成できることに
ほかならない。省略が誤りだというのではなく、省略された内容がこれだという意味である。

\subsection{$f = f_+ + f_-$ は一般には成り立たない}
\label{sec:nonsplit}
...
この非定理は消極的な注意ではない。証明のいくつかの箇所で、単項式の稠密性や
$f = f_+ + f_-$ の形の分解を経由すれば議論が短くなるように見える。そのどれもが
使えない。第 \ref{sec:mirror} 小節で $\beta_d(r_\pi)$ との一致を単項式の稠密性ではなく
...
定理 3.10 の三つの主張は、公理系を満たす\emph{すべての}
...
「この部分は後で埋める」と宣言して先へ進む仕組みがあるが、この開発ではその宣言は
...
本開発はこれ以外の公理を一切導入していない。導入しようとすれば、上の検査が
そのことを検出して失敗する。

これらは形式化を行う際にしばしば使われる抜け道が使われていないことを意味する。
定理を弱めたり、特別な場合に限定したり、都合のよい仮定を追加で置いたりすることは、
これらの検査では検出されない。それを防ぐのは検査ではなく、原論文の主張と Lean の
主張を照合する人間の作業である。この照合は、実装した者とは別の者が行うという
手順のもとで、結果ごとに実施されている。
...
述べられているから、これは定理の範囲の制限ではなく、companion となる結果の
範囲の制限である。

\paragraph{$\Xcl$ の平坦性の符号化。}
定義 \ref{def:Xcl} の端点条件 $F^{(i)}(\pm 1) = 0$ を、形式化では実の意味での
全階の偏導関数が消えることとして表している。$\Oex$ 上可微分で内部で正則な $F$ に
対して、この条件と原論文の複素微分による条件は同値である。複素微分は境界まで
連続に延び、正則関数の実偏導関数はそれらの多項式で書けるからである。この同値性
自体は形式化されていない。$\Xcl$ を\emph{構成する}側では、近似列が実の意味での
条件を満たすことが示されているから問題はない。しかし $\Xcl$ の元を\emph{仮定として}
受け取る補題 3.6, 3.7, 3.9 については、仮定の class が原論文のそれと一致することが
論証されているにとどまり、証明されてはいない。

\paragraph{$\Xcl$ に相対位相を入れていない。}
...
\paragraph{\cite{T26} 第 4 節は範囲外である。}
...
本稿が報告する形式化は、Tener \cite{T26} および Carpi--Raymond--Tanimoto--Tener
\cite{CRTT25} の結果に全面的に依拠している。第 \ref{sec:gaps} 節で述べた
補いの議論は、いずれも原論文の証明の道筋に沿ってその省略部分を明示したもので
あって、新たな定理を主張するものではない。
