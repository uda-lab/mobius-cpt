...
estimate. The Reeh--Schlieder property for the two reference semicircles, which is what makes the
continued operators densely defined, is proved as well. A statement of what the verification does
...
本稿の保存版には、上のコミットにおける形式化のソースの書庫が本稿とともに収められる。
本節と第 \ref{sec:limits} 節の主張は、したがってその書庫に対して確かめられる。
開発リポジトリそのものへのアクセスは前提としない。上のコミット識別子は版を一意に
指定するから、書庫の内容と以下の記述との対応は一意に定まる。
...
出典欄が括弧書きの行は、その主張が原論文の番号つき結果そのものではなく、その証明が用いる
事実、または本稿の開発で導いた帰結であることを示す。
...
第一に、上表のすべての結果について、証明に未完成の箇所がない。証明支援系には、証明の
...
対象として置かれている。置かれているのは「その対象が与えられていない」ことを型の上で表明する項であって、対象
そのものではないから、それを使って何かを証明することはできない。上表のどの結果もそれに
...
どの公理に（推移的に）依存するかを機械的に列挙できる。固定した版では、上表の結果を含む
...
だけでなく定義や、型クラスの具体化として与えたものも含む。列挙の対象がこの \auditcount\ 件で
...
定理のほかに定義や型クラスの具体化を含む。\coveragecount\ はそのうち、公開された定理
...
第 \ref{sec:gaps} 節と第 \ref{sec:status} 節の対応表は、その照合の出発点として書かれている。
