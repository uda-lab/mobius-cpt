...
枠組みにおける PCT 作用素そのものの構成ではない。ユニタリな場合には反ユニタリな対合 $\theta$ が
存在し、その作用がこの等式の右辺と一致する（\cite{RTT22}）。非ユニタリな枠組みで対応する
対合を得るには、不変 Hermite 形式などの追加の構造が要り、それは \cite{T26} 第 4 節の主題である。
定理 3.10 はその議論の入力であって結論ではない。本稿が「PCT 定理」と呼ぶのは、この等式が
PCT 変換の幾何学的内容、すなわち時空の向きの反転と場の順序の逆転を、作用素の等式として
表しており、かつ後続の PCT 対合と Bisognano--Wichmann 性がこれを土台に構成されるからである。
...
要求する。$\Dstar$ は $\Fields$-強位相についての連続双対であり $\Dom$ の点を分離するから、
この要求は情報を失わない。
...
\cite{CRTT25} 補題 2.10 (i) & $t \mapsto \beta_d(v_t)f$、$\theta \mapsto \beta_d(r_\theta)f$ の連続性 \\
 & およびそこから従う $t \mapsto V_t\Phi$ の $\Fields$-強連続性 \\
...
$I_\pm$ について述べられており、その証明も $I_\pm$ に対する稠密性しか使わないから、
