...
\newcommand{\pinnedcommit}{9664674d88e40ba0158d684d28e2153f4809f7f7}
\newcommand{\pinneddate}{2026年8月31日}
\newcommand{\auditcount}{820}
\newcommand{\notedate}{2026年8月31日}
% ------------------------------------------------------------------------------
...
内積もユニタリ性も仮定しない枠組みで証明した。従来の PCT 定理が ユニタリ性と正定値性を
本質的に用いていたのに対し、\cite{T26} が扱うのは非ユニタリな Wightman 型の公理系であり、
...
定理 3.10 までである。次のものは扱わない。Haag--Kastler の意味での共形ネットとその頂点作用素代数との
対応（\cite{CKLW18}）、
...
述べ、主定理を正確に述べたうえで、証明の依存関係を図示する。第 4 節は解析的な芯の上での
...
$\Xcl$ に制限し、そこで公式を直接示す（第 \ref{sec:core} 節）。$\Xcl$ から来る試験関数
だけで作られるベクトル $\varphi_1(f_1)\cdots\varphi_k(f_k)\Omega$ の全体を、以下
\textbf{解析的芯}と呼ぶ。この段階では
...
\node[box, below=7mm of l36] (l37) {補題 3.7\\ 解析的芯上の PCT 公式};
...
\end{tikzpicture}
\caption{証明の依存関係。補題 3.8 は解析的芯を経由せず、公理から直接得られる。
...
本稿の公開時点において、この開発リポジトリは公開されていない。したがって本節と
第 \ref{sec:limits} 節の記述を読者が独立に確かめることは、現時点ではできない。
これらは検証済みの事実の報告であって、読者に検証の手順を示すものではない。
上のコミット識別子は版を一意に指定するから、ソースが利用可能になった時点で、
以下のすべての主張はその版に対して確かめられる。
...
（本稿で導いた帰結） & 解析的芯 $P_\Xcl(I_\pm)\Omega$ の稠密性 \\
\cite{T26} 定義 3.5 & 複素ブースト $\beta_d(v_\tau)$（$d = 0$ の可除特異点を込めて）、半群則 \\
\cite{T26} 補題 3.6 & $\tau \mapsto \beta_d(v_\tau)F|_{I_+}$ と $G_\lambda$ の、$\Strip_{i\pi}$ 上の \\
 & 連続性と内部での正則性 \\
(W3) 橋渡し & 正モードのみをもつ試験関数に対する $\varphi(f)\Omega = 0$ \\
\cite{T26} 補題 3.7 & (i) 解析的芯上の $\Vt_\tau$ の値、(ii) $\tau = i\pi$ における符号 \\
...
上表の結果を含む \auditcount\ 個の命題、すなわち定義・補題・定理として
書き下されたもののすべてについてこの列挙を実行し、現れる公理が
\[
...
\paragraph{\texorpdfstring{補題 3.6 の $\Dom$ 値の形は扱っていない。}{補題 3.6 の D 値の形は扱っていない。}}
補題 \ref{lem:36} について確立されているのは、試験関数の空間に値をとる写像
$\tau \mapsto \beta_d(v_\tau)F|_{I_+}$ の連続性と正則性、および両立汎関数でスカラー化した
$G_\lambda$ の連続性と正則性である。積 $\varphi_1(\beta_{d_1}(v_\tau)F_1|_{I_+})\cdots\Omega$ を
$\Dom$ に値をとる $\tau$ の写像とみたときの、$\Fields$-強位相についての連続性と正則性は
扱っていない。定義 \ref{def:vtilde} が要求するのはスカラー化した形だけであり、第 3 節で
述べたとおり、スカラー化した形から $\Dom$ 値の形が一般に従うわけではない。

\paragraph{\texorpdfstring{$\Xcl$ に相対位相を入れていない。}{解析的試験関数の族に相対位相を入れていない。}}
\cite{T26} 定義 3.2 は制限写像の単射性を用いて $\Xcl$ を $C^\infty(\Circ)$ の部分集合と
みなし、相対位相を入れる。形式化はこの単射性までを証明し、$\Xcl$ 自身には位相を
入れていない。補題 \ref{lem:34} の稠密性は $C^\infty(\Circ)$ の位相について述べられて
おり、$\Xcl$ の内部での収束に関する主張は行っていない。定理 \ref{thm:main} の証明が
必要とするのは制限 $F|_{I_+}$ の側の収束だけだから、この省略は証明に影響しない。
