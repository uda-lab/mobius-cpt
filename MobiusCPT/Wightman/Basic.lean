...
Separate ⟹ joint is proved in `MobiusCPT.Wightman.Continuity` for the concrete
`C^∞(S¹)` of `MobiusCPT.TestFunctions`, so over that model the two readings of
`D*_𝓕` coincide (`WightmanStruct.isCompatible_iff_isCompatibleSep`) and so do the
two readings of regularity (`WightmanStruct.actsRegularly_iff_actsRegularlySep`).
The argument needs the test-function space to be barrelled and first countable,
which the abstract `TestFunctions` interface does not require; over an arbitrary
instance only joint ⟹ separate holds, and `IsCompatible` remains the [T26]
